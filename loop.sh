...
# line 170 std::string descriptorType4match = "DES_HOG"; // DES_BINARY, DES_HOG
...
for i in "${detectorType_arr[@]}"; do
    ...
    if [ ${det_COUNT} -ne 0 ]; then
        ...
        grep -rl "detectorType = \"${detectorType_arr[$((det_COUNT - 1))]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|detectorType = \"${detectorType_arr[$((det_COUNT - 1))]}\"\;|detectorType = \"$i\"\;|g"
    fi
    ...
    for j in "${descriptorType_arr[@]}"; do
        ...
        if [ ${des_COUNT} -ne 0 ]; then
            ...
            # echo -e $j # DEBUG
            grep -rl  "descriptorType = \"${descriptorType_arr[$((des_COUNT - 1))]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|descriptorType = \"${descriptorType_arr[$((des_COUNT - 1))]}\"\;|descriptorType = \"${descriptorType_arr[$((des_COUNT))]}\"\;|g"

            # grep and sed descriptorType_HOG_BIN
            grep -rl "descriptorType4match = \"${desType_HB_arr[$((des_COUNT - 1))]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|descriptorType4match = \"${desType_HB_arr[$((des_COUNT - 1))]}\"\;|descriptorType4match = \"${desType_HB_arr[$((des_COUNT))]}\"\;|g"
        fi
    ...
    des_COUNT=$((des_COUNT - 1))
    grep -rl "descriptorType = \"${descriptorType_arr[${des_COUNT}]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|descriptorType = \"${descriptorType_arr[${des_COUNT}]}\"\;|descriptorType = \"${descriptorType_arr[0]}\"\;|g"

    # grep and sed back to descriptorType_HOG_BIN = "DES_BINARY";
    grep -rl "descriptorType4match = \"${desType_HB_arr[${des_COUNT}]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|descriptorType4match = \"${desType_HB_arr[${des_COUNT}]}\"\;|descriptorType4match = \"${desType_HB_arr[0]}\"\;|g"
...
det_COUNT=$((det_COUNT - 1))
grep -rl "detectorType = \"${detectorType_arr[${det_COUNT}]}\"\;" ./src/MidTermProject_Camera_Student.cpp | xargs sed -i "s|detectorType = \"${detectorType_arr[${det_COUNT}]}\"\;|detectorType = \"${detectorType_arr[0]}\"\;|g"
